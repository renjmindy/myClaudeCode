...
@mcp.tool()
async def remove_background_as_png(input_image_path: str, output_image_path: Optional[str] = None) -> str:
    """Remove the background from an image, keeping only the main subject, and save as PNG.

    Args:
        input_image_path: Path to the input image file.
        output_image_path: Path to save the output PNG. Defaults to ./images/<original_name>_no_bg.png.
    """
    if not os.path.exists(input_image_path):
        return f"Error: Input image file not found: {input_image_path}"
    ...
    try:
        ...
        if not output_image_path:
            ...
            output_dir = "./images"
            os.makedirs(output_dir, exist_ok=True)
            output_image_path = os.path.join(output_dir, f"{base_name}_no_bg.png")
        else:
